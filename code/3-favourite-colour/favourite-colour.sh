...
docker-compose exec broker kafka-topics --create --zookeeper zookeeper:2181 --replication-factor 1 --partitions 2 --topic favourite-colour-input
...
docker-compose exec broker kafka-topics --create --zookeeper zookeeper:2181 --replication-factor 1 --partitions 2 --topic user-keys-and-colours --config cleanup.policy=compact
...
docker-compose exec broker kafka-topics --create --zookeeper zookeeper:2181 --replication-factor 1 --partitions 2 --topic favourite-colour-output  --config cleanup.policy=compact
...
docker-compose exec broker kafka-console-consumer --bootstrap-server localhost:9092 \
    --topic favourite-colour-output \
    --from-beginning \
    --formatter kafka.tools.DefaultMessageFormatter \
    --property print.key=true \
...
docker-compose exec broker kafka-console-producer --broker-list localhost:9092 --topic favourite-colour-input
#
stephane,blue
john,green
stephane,red
